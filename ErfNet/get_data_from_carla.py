...
def get_data(list_IDs):
    ...
    Y = np.empty((len(list_IDs), *(240,320), 1))

    for i, ID in enumerate(list_IDs):
        # Store sample
        temp = ID.replace('/0/', '/1/')
        img = np.array(Image.open(ID).convert("RGB").resize((320, 240), resample = Image.CUBIC))
        label = np.array(Image.open(temp).convert("RGB").resize((320, 240), resample = Image.NEAREST))
